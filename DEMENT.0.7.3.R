...
RunPulse <- function(
  ...
  # Section to adjust EnzProdInduced to uptake. The higher the uptake, the lower the proportion
  # allocated to induced enzyme. This is calculated independently for each enzyme
    # 1) Find the element that regulates each enzyme
    # 3) Match the uptake of each element with each enzyme
    # Uptake must be normalized by biomass as: Taxon_Uptake_C/rsi
    # Update Induce_Enzyme_C by reducing its numbers when the uptake of the
    # corresponding element is high. Use Enz.track to determine which element 
    # to use
    # May use Taxon_Uptake_C/(params["C_min",]*2) as the regulator factor
    # The x axis will go from ~0 to 1
    ...
    if(params["Enz_repress",] == 1){
      ...
      Enz.repres.l <- list(
      C = round(exp(-12*(C.up.pc - 0.75)) / (1 + exp(-12*(C.up.pc - 0.75) )), digits = 4),
      N = round(exp(-12*(N.up.pc - 0.75)) / (1 + exp(-12*(N.up.pc - 0.75) )), digits = 4),
      P = round(exp(-12*(P.up.pc - 0.75)) / (1 + exp(-12*(P.up.pc - 0.75) )), digits = 4),
      NP = round(exp(-12*(NP.up.pc - 0.75)) / (1 + exp(-12*(NP.up.pc - 0.75) )), digits = 4)  # Old rep(1, length(Taxon_Uptake_N))
      )
    ...
    Respiration <- sum(Taxon_Uptake_C*(1-CUE)) + sum(Enzyme_Maint) + sum(Uptake_Maint) 
    Monomers.grid <- sum.grid(Monomers, Mon.names, grid.size)
...
TraitModel <- function(job.time,task.ID){
  ...
  for (i in 1:params["n_enzymes", ]) {
    if(sum(ReqEnz1[, i]) == 0) ReqEnz1[, i] <- sample(probability_vector,
                                                      params["n_substrates",],
                                                      replace = F)
  }
  # Ensuring there are no enzymes that degrade more than one substrate
  for (i in 1:params["n_enzymes", ]) {
    if(sum(ReqEnz1[, i]) > 1) ReqEnz1[, i] <- sample(probability_vector,
                                                      params["n_substrates",],
                                                      replace = F)
  }
  ...
  Uptake_ReqEnz <- matrix(Uptake_ReqEnz1,
                          nrow = n_monomers,
                          ncol = params["n_uptake", ],
                          byrow = T,
                          dimnames = list(sprintf("%s%03d","Mon",1:n_monomers),
  ...
  for (i in 1:params["n_uptake", ]) {
    if(sum(Uptake_ReqEnz[, i]) == 0) Uptake_ReqEnz[, i] <- sample(probability_vector, 
                                                                  n_monomers,
                                                                  replace = F)
  }
  ...
  dimnames(MonomersProduced) <- list(sprintf("%s%03d","Sub",1:params["n_substrates",]),
                                     sprintf("%s%03d","Mon",-1:(n_monomers-2)))
  ...
  genes_per_taxon <- sample(params["Enz_per_taxon_min",]:params["Enz_per_taxon_max",],
                            params["n_taxa",],
                            replace = T)
  ...
  for (i in 1:params["n_taxa",]) {
    if(sum(UptakeGenes[i,3:n_monomers])==0) UptakeGenes[i,3:n_monomers] <- sample(probability_vector,
                                                                                  n_monomers - 2,
                                                                                  replace = F)
  }
