...
RunPulse <- function(
  ...
  for(i_t in 1:params["end_time", ]) { # Loop through time
    ...
    # 
    # 
    # DOC.p <- DOC/(colSums(Microbe.C.cell) + 10^-10)
    # DON.p <- DON/(colSums(Microbe.N.cell) + 10^-10)
    # DOP.p <- DOP/(colSums(Microbe.P.cell) + 10^-10)
    ...
    if(params["Upt_repress",] == 1){
      ...
      if(i_t > 1){
        ...
        # P.adj <- 1 - (exp(-7*(C.up.pc - 0.3))) / (1 + exp(-7*(C.up.pc - 0.3)))
        
        # Upt.repres.l <- list(
        #   C = round(((exp(-10*(C.up.pc - 0.5)) + 0.01) / (1 + exp(-10*(C.up.pc - 0.5) ))), digits = 4),
        #   N = round(((exp(-10*(N.up.pc - 0.5)) + 0.01) / (1 + exp(-10*(N.up.pc - 0.5) ))), digits = 4),
        #   P = round(((exp(-10*(P.up.pc - 0.5)) + 0.01) / (1 + exp(-10*(P.up.pc - 0.5) ))), digits = 4),
        ...
        #  N = Upt.repres.l$N/(Upt.repres.l$C + Upt.repres.l$N + Upt.repres.l$P),
        #  P = Upt.repres.l$P/(Upt.repres.l$C + Upt.repres.l$N + Upt.repres.l$P),
        #  NP = (Upt.repres.l$N + Upt.repres.l$P)/(Upt.repres.l$C + Upt.repres.l$N + Upt.repres.l$P)
        # 
        # )
    ...
    if(params["Enz_repress",] == 1){
      ...
      # TER <- data.frame(
      #   CN = (params["Cfrac_b",]/params["Nfrac_b",])/CUE.ref[1],
      #   CP = (params["Cfrac_b",]/params["Pfrac_b",])/CUE.ref[1],
      #   NP = (params["Nfrac_b",]/params["Pfrac_b",]),
      #   NC = (params["Nfrac_b",]/params["Cfrac_b",])*CUE.ref[1],
      #   PC = (params["Pfrac_b",]/params["Cfrac_b",])*CUE.ref[1],
      #   PN = (params["Pfrac_b",]/params["Nfrac_b",])
      # )
      # 
      # C.up.N <- (Taxon_Uptake_C/(Taxon_Uptake_N + 10^-10))/TER$CN
      # C.up.P <- (Taxon_Uptake_C/(Taxon_Uptake_P + 10^-10))/TER$CP
      # N.up.C <- (Taxon_Uptake_N/(Taxon_Uptake_C + 10^-10))/TER$NC
      # N.up.P <- (Taxon_Uptake_N/(Taxon_Uptake_P + 10^-10))/TER$NP
      # P.up.C <- (Taxon_Uptake_P/(Taxon_Uptake_C + 10^-10))/TER$PC
      # P.up.N <- (Taxon_Uptake_P/(Taxon_Uptake_N + 10^-10))/TER$PN
      # 
      # # Calculating excess of uptake for each element
      # C.up.pc <- apply(cbind(C.up.N, C.up.P), 1, min)
      # N.up.pc <- apply(cbind(N.up.C, N.up.P), 1, min)
      # P.up.pc <- apply(cbind(P.up.C, P.up.N), 1, min)
      # NP.up.pc <- apply(cbind(N.up.pc, P.up.pc), 1, min) # Selecting the lowest between N and P
      ...
      # # Matrix to subset enzyme repression matrix
      # Enz.col <-
      #   do.call("rbind",
      #           list(C = which(colnames(Enz.repres) == "C"),
      #                N = which(colnames(Enz.repres) == "N"),
      #                P = which(colnames(Enz.repres) == "P"),
      #                NP = which(colnames(Enz.repres) == "NP"))[Upt.lim[which(!is.na(Upt.lim))]]
      #   )
      # 
      # 
      # 
      # # Setting limiting element without repression
      # for(i in 1:nrow(Enz.repres[which(!is.na(Upt.lim)),])){
      # 
      #   Enz.repres[which(!is.na(Upt.lim)), ][i, Enz.col[i, ]] <- 1
      # 
      # }
